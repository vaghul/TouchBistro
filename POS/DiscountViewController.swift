...
class DiscountViewController: UITableViewController {
    ...
    @objc func done() {
        dismiss(animated: true) {
            NotificationCenter.default.post(name: .DiscountViewDismissed, object: nil)
        }
    }
}
...
class DiscountViewModel {    
    ...
    func toggleDiscount(at indexPath: IndexPath) {
        var discount = discounts[indexPath.row]
        discount.isEnabled = !discount.isEnabled
        if discount.isEnabled {
            if discount.selectedTime == 0 {
                discount.selectedTime = Date().toMillis()
            }
        }else{
            discount.selectedTime = 0
        }
        discounts[indexPath.row] = discount
    }
}
